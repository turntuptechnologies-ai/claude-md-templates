...
2. **ブランチ作成** — Issue に紐づくブランチで作業する。命名規則: `issue-<番号>/<簡単な説明>`（例: `issue-5/add-deadline-notification`）
3. **実装** — 作業中は適宜 commit・push を行う。**main への直接 push は禁止**
4. **PR 作成** — Issue を参照する（`Closes #5` 等）
5. **コードレビュー** — マージ前にレビューチームが PR をレビューする。セキュリティ観点のチェックを含める（認証トークンの安全な保存、拡張機能の権限スコープ、XSS 等）
